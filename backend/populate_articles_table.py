...
def populate_table(articles_json, category):
    article_number = 0
    while article_number < 20:
        ...
        cur.execute(f"""
        INSERT INTO articles (articleid,author,content,description,publishedat,title,url,urltoimage,category)
        VALUES (
            {article_number},
            '{author}',
            '{content}',
        ...
        """)
        print("NEW ARTICLE "+str(article_number)+" in category "+str(category)+".")
...
for key in categories:
    ...
    populate_table(articles_json,key) # pass articles_json and the category
